...
# Sets up logging to capture debug information and write it to a log file
logging.basicConfig(filename='/home/ubuntu/every_minute_modsec_dump_parsed_debug.log', level=logging.DEBUG,
...
def parse_modsec_logs(file_path, last_position):
    ...
    transactions = re.split(r'---\w+---Z--', logs)
    parsed_logs = []
    ...
    for transaction in transactions:
        ...
        for part in parts:
            ...
            status_match = re.search(r'HTTP/1.1 (\d+)', part)
        ...
        if all(key in log_data for key in ['ip', 'timestamp', 'method', 'path', 'status']):
            # Formats the log into the required format for Fail2ban
            formatted_log = f"{log_data['ip']} - - [{log_data['timestamp']}] \"{log_data['method']} {log_data['path']} HTTP/1.1\" {log_data['status']} \"{log_data.get('user_agent', '-')}\""
...
# Gets the last read position from the position file
last_position = get_last_read_position(position_file)
