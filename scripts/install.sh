...
if [ "$SKIP_ECR" = false ]; then
    ...
    # Sets up pipeline docker cron jobs for checking new images available in AWS ECR 
    echo ""
    echo " * Setting up docker cron jobs for all containers..." | tee -a "$LOGFILE"
...
sudo apt-get autoremove -y > /dev/null 2>> "$LOGFILE"
sudo apt-get clean > /dev/null 2>> "$LOGFILE"
